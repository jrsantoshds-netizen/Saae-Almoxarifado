...
#============================ Config de ambiente para Plotly ==========
st.set_page_config(
    page_title="SAAE Itacoatiara",
    page_icon="💦",
    layout="centered"
)
...
if page == "📊 Dashboard":
    ...
    with col4:
       #================data_mov = pd.to_datetime(df_mov['data_mov']) if not df
        
        ultima_mov = df_mov['data_mov'].max() if not df_mov.empty else None
        
        if ultima_mov is not None:
            # Formato mm/aaaa → 04/2026
            valor = ultima_mov.strftime("%m/%Y") if hasattr(ultima_mov, 'strftime') else str(ultima_mov)[:7]
        else:
            valor = "Sem movimentações"
        ...
        with st.form("form_cadastro",clear_on_submit=True ):
            ...
            if submit:
                if not nome:
                    ...
                    if success:
                        #st.success(msg)
                        st.toast(f" Cadastrado com sucesso!") # Mensagem flutuante
                        st.rerun()
                    else:
                        st.error(msg)
